...
df = pd.read_csv("/Users/salarmuhammadumer/Desktop/sql-talk-app/Sample_Data_Aril_2023.csv")
df["Trip Start"] = pd.to_datetime(df["Trip Start"], format="%d/%m/%Y %H:%M").dt.strftime("%Y-%m-%d %H:%M:%S")
df["Trip End"] = pd.to_datetime(df["Trip End"], format="%d/%m/%Y %H:%M").dt.strftime("%Y-%m-%d %H:%M:%S")
